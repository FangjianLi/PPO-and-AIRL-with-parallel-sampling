...
def q_target_values(mini_batch_rw, mini_batch_done, av, discounted_value):
    ...
    '''
    max_av = tf.math.reduce_max(av, axis=1)

    mini_batch_done_coeff = 1 - tf.cast(mini_batch_done, tf.float32)
    ys = mini_batch_rw + discounted_value*mini_batch_done_coeff* max_av
